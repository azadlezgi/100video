<?php
	...
	if ($_GET['action']=="order") {
		...
		if ($_POST['save']) {
			...
			if ($_GET['action']=="edit") {
				...
				if (dbrows($result_order)) {
					...
					$page_order = $data_order['page_order']+1;
				} else {
					$page_order = 1;
				}
			}

			$page_date = FUSION_TODAY;
			$page_comments = (INT)$_POST['page_comments'];
			$page_ratings = (INT)$_POST['page_ratings'];
			...
			if (dbrows($result)) {
				...
				$page_ratings = $data['page_ratings'];

				$page_alias = $data['seourl_url'];
				$seourl_prefix_strlen =  strlen($settings['seourl_prefix']);
				$seourl_prefix_alias = substr($page_alias, -$seourl_prefix_strlen);
				if ($seourl_prefix_alias==$settings['seourl_prefix']) {
					$page_alias = substr($page_alias, 0, -$seourl_prefix_strlen);
				}
		...
		if (isset($error)) {
			...
			$seourl_url = (empty($page_alias) ? "page_". $page_id . $settings['seourl_prefix'] : $page_alias . $settings['seourl_prefix']);
			...
			if ($viewseourl['seourl_url']==$seourl_url) {
				if (($viewseourl['seourl_filedid']==$seourl_filedid) && ($viewseourl['seourl_component']==$seourl_component)) {
					...
					$seourl_url = "page_". $page_id . $settings['seourl_prefix'];
				}
			}  // Yesli URL YEst
...
?>
	...
	<form name='inputform' method='POST' action='<?php echo FUSION_SELF . $aidlink; ?>&action=<?php echo $_GET['action'];?><?php echo (isset($_GET['id']) && isnum($_GET['id']) ? "&id=". $_GET['id'] : ""); ?>'>
		<input type="hidden" name="page_status" id="page_status" value="<?php echo $page_status; ?>" />
		...
		<table class='form_table'>
			...
			<tr class="seo_tr">
				<td colspan="2" class="seo_tr">
					...
					<input type="text" name="page_title[<?php echo $value['languages_short']; ?>]" id="page_title_<?php echo $value['languages_short']; ?>" value="<?php echo $page_title[$value['languages_short']]; ?>" class="textbox" style="width:100%;" /><br />
			...
			<tr class="seo_tr">
				<td colspan="2" class="seo_tr">
					...
					<?php foreach ($languages as $key => $value) { ?>
					<?php if ($lang_say>1) { ?><span class="local_name lang_<?php echo $value['languages_short']; ?>"><?php echo $value['languages_name']; ?></span><?php } ?>
					<input type="text" name="page_description[<?php echo $value['languages_short']; ?>]" id="page_description_<?php echo $value['languages_short']; ?>" value="<?php echo $page_description[$value['languages_short']]; ?>" class="textbox" style="width:100%;" /><br />
					<?php } // foreach languages ?>
				</td>
			</tr>
			<tr class="seo_tr">
				<td colspan="2" class="seo_tr">
					...
					<?php foreach ($languages as $key => $value) { ?>
					<?php if ($lang_say>1) { ?><span class="local_name lang_<?php echo $value['languages_short']; ?>"><?php echo $value['languages_name']; ?></span><?php } ?>
					<input type="text" name="page_keywords[<?php echo $value['languages_short']; ?>]" id="page_keywords_<?php echo $value['languages_short']; ?>" value="<?php echo $page_keywords[$value['languages_short']]; ?>" class="textbox" style="width:100%;" /><br />
					<?php } // foreach languages ?>
				</td>
			</tr>
			<tr class="seo_tr">
				<td colspan="2" class="seo_tr">
					...
					<?php foreach ($languages as $key => $value) { ?>
					<?php if ($lang_say>1) { ?><span class="local_name lang_<?php echo $value['languages_short']; ?>"><?php echo $value['languages_name']; ?></span><?php } ?>
					<input type="text" name="page_h1[<?php echo $value['languages_short']; ?>]" id="page_h1_<?php echo $value['languages_short']; ?>" value="<?php echo $page_h1[$value['languages_short']]; ?>" class="textbox" style="width:100%;" /><br />
					<?php } // foreach languages ?>
				</td>
			</tr>
			<tr class="seo_tr">
				<td colspan="2">
					...
					<input readonly type="text" name="page_siteurl" id="page_siteurl" value="<?php echo $settings['siteurl']; ?>" class="textbox" style="width:150px;" />
					<input type="text" name="page_alias" id="page_alias" value="<?php echo $page_alias; ?>" class="textbox" style="width:430px;" />
					<input readonly type="text" name="seourl_prefix" id="seourl_prefix" value="<?php echo $settings['seourl_prefix']; ?>" class="textbox" style="width:50px;" />
				</td>
			</tr>
			<tr class="seo_tr">
				<td colspan="2"></td>
			...
			<tr>
				<td colspan="2">
					...
					<?php foreach ($languages as $key => $value) { ?>
					<?php if ($lang_say>1) { ?><span class="local_name lang_<?php echo $value['languages_short']; ?>"><?php echo $value['languages_name']; ?></span><?php } ?>
					<input type="text" name="page_name[<?php echo $value['languages_short']; ?>]" id="page_name_<?php echo $value['languages_short']; ?>" value="<?php echo $page_name[$value['languages_short']]; ?>" class="textbox" style="width:100%;" /><br />
					<?php } // foreach languages ?>
				</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					...
					<?php foreach ($languages as $key => $value) { ?>
					<?php if ($lang_say>1) { ?><span class="local_name lang_<?php echo $value['languages_short']; ?>"><?php echo $value['languages_name']; ?></span><?php } ?>
					<textarea id="editor<?php echo $value['languages_id']; ?>" name="page_content[<?php echo $value['languages_short']; ?>]" id="page_content<?php echo $value['languages_short']; ?>" class="textareabox" cols="95" rows="15" style="width:100%"><?php echo $page_content[$value['languages_short']]; ?></textarea><br />
					<?php } // foreach languages ?>
				</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					<?php echo display_html("inputform", "page_content", true, true, true, IMAGES_N); ?>
				</td>
			</tr>
			<?php } ?>
			<tr>
				<td>
					...
					<select name="page_access" id="page_access" class="selectbox" style="width:200px;">
						<?php echo $access_opts; ?>
					</select>
				</td>
				<td>
				 <?php if ($settings['comments_enabled']) { ?> <label><input type="checkbox" name="page_comments" value="1"<?php echo ($page_comments ? " checked" : ""); ?> /> <?php echo $locale['512']; ?></label><br /> <?php } ?>
					<?php if ($settings['ratings_enabled']) { ?> <label><input type="checkbox" name="page_ratings" value="1"<?php echo ($page_ratings ? " checked" : ""); ?> /> <?php echo $locale['513']; ?></label><br /> <?php } ?>
				</td>
			</tr>
			<tr>
...
?>
	...
	?>
			<tr id="listItem_<?php echo $data['page_id']; ?>">
				...
				<td class="status">
					<a href="<?php echo FUSION_SELF . $aidlink; ?>&action=status&id=<?php echo $data['page_id']; ?>&status=<?php echo $data['page_status']; ?>" title="<?php echo ($data['page_status'] ? $locale['411'] : $locale['412']); ?>"><img src="<?php echo IMAGES; ?>status/status_<?php echo $data['page_status']; ?>.png" alt="<?php echo ($data['page_id'] ? $locale['411'] : $locale['412']); ?>"></a>
				</td>
